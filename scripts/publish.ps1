...
<#
.SYNOPSIS
    Build every loader and PUBLISH the Blackboard release to CurseForge (asks to confirm first).
...
.DESCRIPTION
    ...
    Tip: run .\scripts\dryrun.ps1 first to validate without uploading.
...
param(
    [switch]$Yes,
    # Optional path to a JDK 21 for the Gradle daemon. Omit to use $env:JAVA_HOME or the repo's
    # gradle/gradle-daemon-jvm.properties (toolchainVersion=21). Nothing machine-specific is baked in.
    [string]$JavaHome
)
...
Write-Host ""
Write-Host "  About to PUBLISH to CurseForge (project $projectId):" -ForegroundColor Yellow
if ($nodes.Count -gt 0) {
    foreach ($node in $nodes) {
        $loader = $node.Substring($node.LastIndexOf('-') + 1)
        Write-Host "    - $modId-$loader-$version+$node.jar"
    }
} else {
...
Write-Host "== Publishing Blackboard $version to CurseForge ==" -ForegroundColor Cyan
...
Write-Host "Published Blackboard $version to CurseForge successfully." -ForegroundColor Green
