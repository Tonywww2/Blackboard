...
> - 现状（2026-07-01）：**P1–P7 全部完成 + Datagen 标签工具落地**——两节点 `build` 均绿、单测通过、Forge `runClient` 实测进入游戏世界（`[blackboard/]: Blackboard loaded`）、`runData` 实测生成方块标签。仅 **P8-A（可选 AUI 客户端渲染）未做**。原「默认黑板 `ByTag(DEFAULT)` 无候选」的端到端待补项**已修复**：内置生成器（P3-D）已补 `DEFAULT` 标签，默认黑板类型可实际出题（见 P5-B / Datagen 行）。
...
| 5 | P5-B 内置黑板类型 | ✅ | Agent3 | `DEFAULT_TYPE`（`blackboard:default`）：`ByTag(DEFAULT)`/`weightedRandomSelect`/`onSolved=defaultReward`/`rewards/default`/`DefaultAnswerFormat`/不限次；`ALL`+`register()`；两节点 build 绿。~~⚠ DEFAULT 标签暂无生成器~~ ✅ 已由「Datagen 标签生成」任务补齐（6 个生成器均加 `DEFAULT` 标签，默认板可正常出题） |
...
| 8 | P8-A AUI 渲染接入（可选） | ⬜ | — | 新建 `client/{BlackboardClient,ApricityBlackboardRenderer}`；注入 `BlackboardRendering.renderer`（触发点 `BlackboardBlockEntity.refreshClientRender` 已就绪）；接 ApricityUI（依赖坐标待补）。渲染器接口目前仅 `render()`，销毁回收待议 |
| 7 | P7-C 生成器热重载指令 | ✅ | Agent1 | `/blackboard reload`（权限 2）：`SimpleRegistry.reopen()` + 可重载事件 `BlackboardEvents.REGISTER_GENERATORS` + `core.GeneratorReload`（启动基线快照 + 可重载层重建）；`GeneratorReloadTest` 3/3。KubeJS 启动脚本受其 startup 限制不随命令刷新（由基线保留，改动需重启）。两节点 build 绿（Agent1 验证收尾） |
| 工具 | Datagen 标签生成 | ✅ | Agent1 | 新增 `data/BlackboardDataGen.kt`（KLF `@EventBusSubscriber` 接 `GatherDataEvent`，自写原始 JSON 的 `DataProvider`，避开各版本 `TagsProvider`/`saveStable` 签名差异）生成方块标签 `#blackboard:blackboards`；`build.gradle.kts` 加 `runData`（`data()` + `--all --mod blackboard`），Forge 实测生成成功；标签双路径 `tags/blocks`(1.20.1)/`tags/block`(1.21.1) 随 jar 发运。**另**：生成器分类标签（`math`/`text`/`default`）属模组内部注册表（非 MC 数据标签），故在 `BuiltinGenerators` 代码给 6 个生成器补 `DEFAULT`，修复默认板 `ByTag(DEFAULT)` 选不到题（见 P5-B）；`BuiltinGeneratorsTest` 增断言。两节点 build 绿、`runData` 实测 |
| 扩展 | 删除 / 禁用现有生成器（Mod + KubeJS） | ✅ | Agent1 | `RegisterGeneratorsEvent.remove(id)`/`disable(id)`（协调器 `GeneratorReload.rebuildInto(...,disabled)` 恢复基线/追加时均跳过被禁用 id，能禁用内置/基线生成器、随 `/blackboard reload` 一致）；`SimpleRegistry.unregister(id)`（仅未冻结时，移除条目+标签索引+反查）；KubeJS `RegisterGeneratorsKubeEvent.remove(id)`（startup 冻结前 unregister，故不随 reload 复活）。`GeneratorReloadTest` 增禁用用例（两节点 4/4）、`SimpleRegistryTest` 增 unregister 三用例（两节点 9/9）。两节点 build 绿 |
| 闭环 | 黑板类型世界内接线（分配类型 + 触发出题） | ✅ | Agent1 | 新增 `BlackboardApi.DEFAULT_TYPE_ID`；`BlackboardManager.onPlaced(be,player)`（放置时分配 boardId+默认类型、`track`、`generateQuestion`）由 `BlackboardBlock.setPlacedBy` 调用；`PlatformEvents` 加 `/blackboard settype <typeId>` 与 `/blackboard generate`（视线+`ClipContext`/`Level.clip` 射线定位注视的黑板，两版通用）。至此放置即出题、自定义类型/池可在世界内使用。两节点 build 绿 |
| 扩展 | 微积分题库模块（`CalculusExpr`/`CalculusProblems`/`CalculusGenerators`） | ✅ | Agent1 | 零依赖自写表达式 AST + 符号求导 + 逆向积分出题 + 数值抽样判等（+C 两点差）。新增 `BlackboardTags.CALCULUS` 与内置类型 `blackboard:calculus`（`ByTag`）。数值题（求导取值/定积分/极限，整数答案走 `Validators.number`）+ 符号题（A–F 求导 / H 不定积分，`EXPRESSION` 版 `validate` 走抽样等价）。LaTeX 归一化 + infix 递归下降解析器（含隐式乘法、变量指数 `e^x`）。M1–M6 六模块各带单测共 22 道，两节点 build 绿。详见 [calculus-module](../references/calculus-module.md) 与 [calculus-tasks](calculus-tasks.md)。i18n 题干键延后至 P8-A 渲染阶段 |
...
| **P7-C 生成器热重载指令** 🟨 ✅ **已完成** | `…/core/GeneratorReload.kt` + `…/api/event/RegisterGeneratorsEvent.kt`（**跨契约扩展**：`api/event/BlackboardEvents.kt` 加 `REGISTER_GENERATORS`、`api/registry/SimpleRegistry.kt` 加 `reopen()`、`platform/PlatformEvents.kt` 加 `initialLoad`+`/blackboard reload`） | P7-A、P3-A、P1-B | [internal-core-api §2/§3/§10](../references/internal-core-api.md) | ✅ 两节点 build 绿、单测通过（`GeneratorReloadTest` 3：基线保留+可重载增改、`reopen` 清空解冻、同 id 基线优先）。两层模型：启动基线（init/KubeJS-startup 直接注册者，`initialLoad` 快照）+ 可重载层（`REGISTER_GENERATORS` 贡献）；`/blackboard reload`（权限 2，`RegisterCommandsEvent`）重开注册表→恢复基线→追加可重载→冻结；类型只在 `initialLoad` 冻结。 |

> 隔离说明：P7-A 只管**生命周期/聊天总线**与冻结时机；P7-B 只管 **mod 总线注册**与内容/渲染装配。两者文件不同、职责不交叉、互不调用，可并行。冻结统一由 P7-A 负责（P7-B 不碰冻结）。P7-C（生成器热重载，后加的跨切面任务）扩展了 `BlackboardEvents`(P3-A)/`SimpleRegistry`(P1-B)/`PlatformEvents`(P7-A) 三处，并把 P7-A 的 `onServerAboutToStart` 从 `freezeRegistries()` 改为 `GeneratorReload.initialLoad()`（内部完成冻结）。
...
## 修订记录
- 2026-07-02：Agent1 完成 **微积分题库模块**（扩展任务，M1–M6 六模块）：零依赖自写实现，`builtin/calculus/{CalculusExpr,CalculusProblems,CalculusGenerators}.kt` 三类拆分（表达式 AST/渲染/解析 · 出题逻辑 · 装配）。M1 AST（求导/化简/`toInfix`/`toLatex`）；M2 数值题型 G/I/J（求导取值/定积分/极限，均整数答案走 `Validators.number`）；M3 数值装配 + 内置类型 `blackboard:calculus`（`ByTag(CALCULUS)`）+ 入口接线（`Blackboard.kt` 冻结前 `register()`/`registerType()`）；M4 `normalizeLatex` + infix 递归下降解析器（隐式乘法、右结合 `^`、一元负高于 `^`、变量指数 `e^x`→`e^{f·ln a}`）+ `CalculusEvaluator` + 抽样判等 `sampleEqual`/`sampleEqualUpToConst`（+C 两点差）；M5 符号题型 A–F 求导 + H 不定积分（逆向造原函数）；M6 `EXPRESSION` 版 `validate`（读 `answer`/`lo`/`hi`/`intg`，`intg?消C:逐点`，解析失败→`Invalid`）+ 符号生成器并入 `ALL`。新增 `BlackboardApi.BlackboardTags.CALCULUS`。单测 6 文件共 22 道（`CalculusExprTest`6/`CalculusProblemsTest`5/`CalculusGeneratorsTest`4/`CalculusParserTest`7/`CalculusSymbolicTest`4/`CalculusSymbolicGenTest`4），两节点（1.20.1-forge / 1.21.1-neoforge）`build` 全绿。i18n 题干键（M3.7/M6.3）延后至 P8-A 渲染阶段。详见 [calculus-module](../references/calculus-module.md) 与 [calculus-tasks](calculus-tasks.md)。
...
- 2026-07-01：**P7-C（生成器热重载指令）** 验证收尾（Agent1；实现已就绪）：`api/event/RegisterGeneratorsEvent.kt` + `core/GeneratorReload.kt`（`initialLoad` 快照启动基线、`reload` 经 `/blackboard reload` 重建）；跨契约扩展 `BlackboardEvents.REGISTER_GENERATORS`(P3-A)、`SimpleRegistry.reopen()`(P1-B)、`PlatformEvents`(P7-A：`initialLoad` 取代原 `freezeRegistries` 调用 + `RegisterCommandsEvent` 注册 `/blackboard reload` 权限 2)。两层模型（基线保留 init/KubeJS-startup 注册者、可重载层重跑 `REGISTER_GENERATORS`）。确认两节点 build 绿、`GeneratorReloadTest` 3/3；补 §4 条目、看板负责人与本记录。
...
- 2026-07-01：Agent1 新增**项目主 README**（`README.md`）：基础介绍 + 构建/玩法 + 「用 Mod（Java/KT）扩展」（添加/删除 QuestionGenerator、注册黑板类型、使用不同生成器库/池）与「用 KubeJS（6/7）扩展」（添加/删除 QuestionGenerator）；示例按真实 API 校对（`QuestionGenerator.builder`/`Questions.builder`/`BlackboardType.builder`/`GeneratorPool`/`BlackboardRegistries.register`/`BlackboardEvents.*`/KubeJS `BlackboardEvents` 事件组）。核对中发现两处与请求直接相关的**未实现缺口**，已登记为上方看板任务：①**删除/禁用现有生成器**（Mod + KubeJS 均无删除入口；仅 `SELECT_GENERATOR` 能按次排除候选）；②**黑板类型世界内接线**（`setBlackboardType`/`generateQuestion` 无调用点，放置不分配类型、无出题触发，自定义类型/池暂无法端到端游玩）。README 「现状与限制」小节列出状态矩阵并回指本文件。
- 2026-07-01：Agent1 完成 **删除/禁用生成器（Mod + KubeJS）**：`SimpleRegistry.unregister(id)`（未冻结时移除条目/标签索引/反查）；`RegisterGeneratorsEvent.remove(id)`=`disable(id)`（收集禁用集）；`GeneratorReload.rebuildInto(...,disabled=emptySet())` 在恢复基线与追加可重载层时都跳过禁用 id（随 `/blackboard reload` 一致、且能禁用内置/基线生成器）；KubeJS `RegisterGeneratorsKubeEvent.remove(id)`（startup 冻结前直接 unregister，故不随 reload 复活）。`GeneratorReloadTest` 增「禁用基线+可重载」用例（两节点 4/4）、`SimpleRegistryTest` 增 unregister 三用例（两节点 9/9）。两节点 build 绿。
- 2026-07-01：Agent1 完成 **黑板类型世界内接线**：新增 `BlackboardApi.DEFAULT_TYPE_ID`（`blackboard:default`）；`BlackboardManager.onPlaced(be,player)`（放置时分配 boardId+默认类型、`track`、`generateQuestion`）由 `BlackboardBlock.setPlacedBy` 调用（取代原仅 `assignBoardIdIfAbsent`）；`PlatformEvents` 加 `/blackboard settype <typeId>`（校验类型已注册→改注视黑板的类型→重出题）与 `/blackboard generate`（重出题），用 `player` 视线 + `ClipContext`/`Level.clip` 射线定位注视的黑板（两版通用、无需 Stonecutter 分支）。至此放置即出题、自定义类型/池可在世界内使用。两节点 build 绿。
- 2026-07-01：Agent1 **README Java 化 + 扩充**：给公共 builder/工厂/注册表/事件加 `@JvmStatic`/`@JvmField`（`QuestionGenerator/BlackboardType/Questions.builder`、`AnswerResult.correct/incorrect/invalid`、`BlackboardRegistries.QUESTION_GENERATORS/BLACKBOARD_TYPES`、`BlackboardEvents.*`）使 Java 调用干净（不影响 Kotlin）；新增 `src/test/java/.../JavaApiExampleTest.java`（两节点 3/3）编译校验 README 的 Java 片段。README 每个用法补齐 Kotlin+完整 Java、内置方法+自定义箭头函数两种写法，新增「手动题库与 i18n」小节（`Component.translatable` 键+`%n$s` 占位符）；「现状与限制」表更新为「删除/禁用」「世界内使用」已实现。
- 2026-07-01：Agent1 **健壮性：题库为空不崩溃**：`selectGenerator` 由「空候选 `require` 抛异常」改为**返回 `null`**；`BlackboardManager.generateQuestion` 遇 `null` 时 `warn` 并跳过出题（不再打 error 栈）。移除全部生成器后放置 / `/blackboard generate` / 答题均优雅降级（板无题→答题回「暂无题目」）。`SelectionTest` 增空池返回 null 用例（两节点 5/5）。
- 2026-07-01：Agent1 新增 **「创建新黑板方块并绑定类型」扩展点**：新增 `content/BlackboardBoards.kt`——`register(path, typeId[, props])` 注册 `blackboard:<path>` 新黑板方块+`BlockItem` 并绑定类型（另有 `register(path, typeId:String)` 便于 KubeJS）、`bind(block, blockId, typeId)` 收编外部命名空间的黑板方块；`ModBlockEntities` 的 BE 类型有效方块集与 `ModCreativeTabs` 展示项改为取自 `BlackboardBoards`（基础黑板为种子，基础行为不变）；`BlackboardManager.onPlaced` 放置时按方块 id 查绑定类型（`BuiltInRegistries.BLOCK.getKey`），未绑定回退 `DEFAULT_TYPE_ID`；`ModBlocks/ModItems.REGISTER` 由 `private` 改 `internal` 供扩展点复用。须在注册表事件之前调用（mod 构造期 / KubeJS startup，`DeferredRegister` 尚未结算）。两节点 build 绿。README 加 Mod/KubeJS 两版示例并整理案例结构/排序（含手动题库的 KubeJS 版）。
