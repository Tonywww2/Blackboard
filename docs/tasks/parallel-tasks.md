...
- 2026-07-01：Agent1 完成 **Datagen 标签生成工具 + 修复默认板选题**：①新增 `data/BlackboardDataGen.kt`——KLF `@EventBusSubscriber` 接 `GatherDataEvent`，用自写原始 JSON 的 `DataProvider`（`CachedOutput.writeIfNeeded` + Guava sha256）生成方块标签 `#blackboard:blackboards`（含 `blackboard:blackboard`），避开各版本 `TagsProvider`/`DataProvider.saveStable` 签名差异；仅 `GatherDataEvent`/`SubscribeEvent`/`EventBusSubscriber` import 与标签目录名（`tags/blocks`(1.20.1)→`tags/block`(1.21.1)，1.21 起 blocks→block 单数化）经 Stonecutter 隔离。②`build.gradle.kts` 加 `runData`：`runConfigs.create("data"){ data(); programArgs("--all","--mod", modId) }`——`--mod` 必须显式传（否则 Forge `GatherDataEvent` 不为本 mod 触发，实测「All providers took: 0 ms」无产出）；且 `modId` 须提到顶层 `val`（在 `runConfigs` 块内 `property(...)` 会解析到 `RunConfigSettings` 成员而非 `Project`，曾误传 `--mod kotlin.Unit`）。Forge `runData` 实测生成 `run/generated/data/blackboard/tags/blocks/blackboards.json`。③按 P1-F loot 表双路径先例，把标签手工提交进 `src/main/resources/data/blackboard/tags/{blocks,block}/blackboards.json` 随两节点 jar 发运。④**修复 P5-B 端到端缺口**：`QuestionGenerator` 的分类标签属模组内部注册表（`SimpleRegistry.tagIndex`，非 MC 数据标签，datagen 管不到），故在 `BuiltinGenerators` 代码给 6 个生成器补 `BlackboardTags.DEFAULT`（math/text 均 `.tag(..., DEFAULT)`），默认板 `ByTag(DEFAULT)` 由此有候选；`BuiltinGeneratorsTest` 增「全部含 DEFAULT 标签且 `byTag(DEFAULT)` 数=ALL 数」断言。两节点 build 绿、`BuiltinGeneratorsTest` 3/3、Forge `runData` 实测产出正确 JSON。
