...
| 扩展 | 删除 / 禁用现有生成器（Mod + KubeJS） | ⬜ | — | 为「删除/禁用已注册的 QuestionGenerator」补入口。Mod 侧建议：`RegisterGeneratorsEvent` 加 `remove(id)`/`disable(id)`（协调器 `GeneratorReload.rebuildInto` 恢复基线时跳过被禁用 id）+ 可选 `SimpleRegistry.unregister(id)`（仅未冻结时）；KubeJS 侧：`RegisterGeneratorsKubeEvent` 加 `remove(id)`（startup 期、冻结前从注册表移除）。需覆盖内置/他 mod 生成器的禁用、与 `/blackboard reload` 一致、并补单测。当前仅 `SELECT_GENERATOR` 能按次排除候选（非注册表级删除）。README「删除现有的 QuestionGenerator」小节已标注此缺口 |
| 闭环 | 黑板类型世界内接线（分配类型 + 触发出题） | ⬜ | — | 现状：`BlackboardBlockEntity.setBlackboardType` 与 `BlackboardManager.generateQuestion` 均无调用点——放置黑板时不分配 `BlackboardType`（`blackboardTypeId` 恒为 null→`generateQuestion` 直接返回 null），且无「放置/交互→出题」钩子，故自定义/非默认黑板类型无法端到端游玩。需：放置时按物品/方块变体/GUI/指令分配类型（至少默认给 `DEFAULT_TYPE`），并在放置或首次交互时调 `generateQuestion`。README「注册新的黑板类型」「使用不同的 QuestionGenerator 库」小节已标注此缺口 |
