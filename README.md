...
模组自带下列题目生成器，随模组启动注册（属**启动基线**，可被事件 / KubeJS / `/blackboard reload` 覆盖或禁用，见[扩展](#用-modjavakotlin-扩展)）。id 均在 `blackboard:` 命名空间下。

### 数值 / 文字题（`#blackboard:default` 池）

这些生成器同时带 `default` 标签，因此放置的默认黑板（`blackboard:default` 类型）会从中按权重随机出题。数值题用 `Validators.number` 判定、文字题用 `Validators.text`（`yes`/`no` 大小写不敏感）。

| 生成器 id | 标签 | 权重 | 题面示例 | 答案 | 判题 |
| --- | --- | --- | --- | --- | --- |
| `addition` | `math` `default` | 10 | `47 + 8 = ?`（两数 1–99） | `55` | 数值 |
| `subtraction` | `math` `default` | 10 | `73 - 26 = ?`（结果非负） | `47` | 数值 |
| `multiplication` | `math` `default` | 8 | `7 * 9 = ?`（两数 2–12） | `63` | 数值 |
| `division` | `math` `default` | 6 | `56 / 8 = ?`（整除，无余数） | `7` | 数值 |
| `square` | `math` `default` | 5 | `13^2 = ?`（底数 2–20） | `169` | 数值 |
| `true_false_sum` | `text` `default` | 5 | `Is 12 + 30 = 42? (yes/no)` | `yes` / `no` | 文字 |

### 微积分题（`#blackboard:calculus` 池）

微积分生成器**只带 `calculus` 标签**（不进默认池），由内置黑板类型 `blackboard:calculus` 出题。启用方式：放置黑板后看向它执行 `/blackboard settype blackboard:calculus`（见[玩法](#玩法)）。数值题走数值判题；**符号题**按数值抽样等价判定（不定积分自动容忍 `+C`）。

| 生成器 id | 权重 | 题型 | 题面示例（infix） | 答案形式 | 判题 |
| --- | --- | --- | --- | --- | --- |
| `calc_deriv_at` | 6 | 某点导数值 | `d/dx(x^3-2*x) \| x=2` | 整数，如 `10` | 数值 |
| `calc_def_integral` | 5 | 定积分 | `∫[0,2] (3*x^2+1) dx` | 整数，如 `10` | 数值 |
| `calc_limit` | 5 | 极限（代入 / 因式相消 / `x→∞` 有理式） | `lim x→3 (x^2-9)/(x-3)` | 整数，如 `6` | 数值 |
